...
function IndexComprador() {
    ...
    useEffect(() => {
        axios.get("http://localhost:3005/Vendedor")
            .then(response => {
                const vendorNames = response.data.map(v => v.Nombre);
    ...
    const getFilteredProducts = () => {
        return products.filter((product) => {
            ...
            const matchesPrice = priceFilter === "" ||
                (priceFilter === "Menos de $2000" && parseInt(product.price) < 2000) ||
                (priceFilter === "$2100 - $4900" &&
                    parseInt(product.price) >= 2100 &&
                    parseInt(product.price) <= 4900) ||
                (priceFilter === "Más de $5100" && parseInt(product.price) > 5100);

            const matchesVendor = vendorFilter === "" || product.madeBy === vendorFilter;
